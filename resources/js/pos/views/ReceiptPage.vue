...
const receipt = getCache(`pos:receipt:${refKey}`, null) || getCache("pos:receipt:last", null) || {};
...
<template>
  <div class="receipt-print-root">
    ...
    <div class="px-6 py-4 border-t flex gap-2 print:hidden">
      <button class="px-3 py-2 rounded bg-gray-100 border" @click="goHome">Nueva venta</button>
      <button class="px-3 py-2 rounded bg-blue-600 text-white" @click="() => window.print()">Imprimir</button>
    </div>
    </div>
  </div>
</template>
...
@media print {
  #app > *:not(.receipt-print-root) { display: none !important; }
