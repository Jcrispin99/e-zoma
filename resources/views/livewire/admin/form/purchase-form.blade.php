    ...
    <x-wire-card class="mb-3">
        <div class="flex items-start justify-between gap-3">
            ...
            <div>
                ...
                @if($payment_status)
                <x-wire-badge :label="str($payment_status)->upper()"
                    :color="$payment_status === 'paid' ? 'emerald' : ($payment_status === 'partial' ? 'amber' : 'slate')" />
                @endif
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <x-wire-dropdown icon="bars-3" align="right">
                    ...
                    @if(($payment_status ?? 'unpaid') === 'unpaid')
                    <x-wire-dropdown.item label="Registrar pago" wire:click="markPaid" />
                    @else
                    <x-wire-dropdown.item label="Anular pago" wire:click="markUnpaid" />
                    @endif
