<div class="space-y-6" x-data="{ activeTab: 'general' }">
    <div class="flex items-center justify-between">
        ...
        <h2 class="text-lg font-semibold">Editar variante</h2>
    </div>
    ...
    <form wire:submit.prevent="save" class="space-y-6">
        <div x-show="activeTab === 'general'" class="space-y-4">
            <x-wire-input label="Nombre" placeholder="Nombre del producto" value="{{ $productName }}" readonly />
            <x-wire-input label="SKU" wire:model.defer="sku" placeholder="SKU del producto" />
            <x-wire-input label="Código de Barras" wire:model.defer="barcode" placeholder="Código de barras (opcional)" />
            <x-wire-input type="number" label="Precio" wire:model.defer="price" placeholder="Precio del producto" step="0.01" />
        </div>
    </form>

    <!-- Pestaña de Imágenes (fuera del <form>) -->
    <div x-show="activeTab === 'images'" class="space-y-4">
        @if(!$isEditing)
        <div class="p-4 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded">
            Primero guarda la variante para poder subir y gestionar imágenes.
        </div>
        @else
        @push('css')
        ...
        <form action="{{ route('admin.variants.dropzone', $variant?->id) }}" id="variant-dropzone"
