{{-- Vista única para mostrar/descargar la Venta (admin y minimal) --}}
@php($sale = $sale ?? $model ?? null)
@php($useLayout = isset($useLayout) ? (bool) $useLayout : auth()->check())
@php($isPublic = (bool) ($isPublic ?? false))

@if($useLayout)
<x-admin-layout title="Venta" :breadcrumbs="[
    [
        'name' => 'Dashboard',
    ...
    <x-wire-card class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <x-wire-button teal :href="URL::signedRoute('public.sales.pdf.view', ['sale' => $sale])">
                    Ver público
                </x-wire-button>
                <x-wire-button light gray :href="route('admin.sales.edit', $sale)">
                    Volver a editar
                </x-wire-button>
            </div>
        </div>
        @include('pdf._styles')
        @include('pdf._document_generic', ['model' => $sale, 'documentLabel' => 'Venta'])
